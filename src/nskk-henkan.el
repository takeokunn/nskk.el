...
(declare-function nskk-state-romaji-buffer "nskk-state")
(declare-function nskk-state-set-romaji-buffer "nskk-state")
(declare-function nskk-state-conversion-start-marker "nskk-state")
(declare-function nskk-state-set-conversion-start-marker "nskk-state")
(declare-function nskk-state-conversion-overlay "nskk-state")
(declare-function nskk-state-set-conversion-overlay "nskk-state")
(declare-function nskk-state-pending-romaji-overlay "nskk-state")
(declare-function nskk-state-set-pending-romaji-overlay "nskk-state")
(declare-function nskk-state-dcomp-multiple-overlay "nskk-state")
(declare-function nskk-state-set-dcomp-multiple-overlay "nskk-state")
(declare-function nskk-state-henkan-count "nskk-state")
(declare-function nskk-state-set-henkan-count "nskk-state")
(declare-function nskk-state-registration-depth "nskk-state")
(declare-function nskk-state-set-registration-depth "nskk-state")
...
(declare-function nskk-numeric-mode "nskk-input" ())
(declare-function nskk-set-numeric-mode "nskk-input" (value))
...
(declare-function nskk-set-azik-sokuon-okuri-kana-pending "nskk-input" (value))
...
(declare-function nskk-set-sticky-shift-pending "nskk-input" (value))
...
The saved range follows intervening edits until another commit replaces
the record or `nskk-undo-kakutei' consumes it.")

(defun nskk-last-kakutei-record ()
...
CANDIDATES and INDEX are passed to each hook."
  (nskk--run-presentation-actions 'show-list #'funcall)
  (run-hook-with-args
...
PREVIOUS-PHASE, and PREVIOUS-COUNT are restored after an `error' or `quit'."
  (condition-case condition
    ...
    ((error quit)
     ...
     (nskk-state-set-henkan-count previous-count)
...
has been restored."
  (let (first-condition)
    (nskk--with-cleanup-runner run-cleanup first-condition
      ...
      (run-cleanup (lambda () (nskk-state-set-henkan-count 0)))
...
(defun nskk-henkan-candidate-list-active ()
  "Return the current value of `nskk--henkan-candidate-list-active'."
  nskk--henkan-candidate-list-active)

(defun nskk-set-henkan-candidate-list-active (value)
  "Set `nskk--henkan-candidate-list-active' to VALUE and return VALUE."
  (setq nskk--henkan-candidate-list-active value))
...
(defun nskk-insert-marker (marker)
  "Insert MARKER string without triggering modification hooks."
  (let ((inhibit-modification-hooks t))
    (insert marker)))
...
(defun/done nskk--update-overlay (start end text)
  ...
  (let ((ov (nskk-state-conversion-overlay)))
    (if (overlayp ov)
        (move-overlay ov start end (current-buffer))
      (setq ov (make-overlay start end))
      (nskk-state-set-conversion-overlay ov))
    (cl-loop for (prop val) on (list 'display text 'face 'highlight) by #'cddr
...
the characters are buffered in `nskk-state-romaji-buffer', not yet committed."
  (when (and (stringp text) (not (string-empty-p text)))
    (let ((ov (nskk-state-pending-romaji-overlay)))
      (if (overlayp ov)
          (move-overlay ov (point) (point) (current-buffer))
        (setq ov (make-overlay (point) (point)))
        (nskk-state-set-pending-romaji-overlay ov))
      (cl-loop for (prop val) on (list 'after-string text) by #'cddr
               do (overlay-put ov prop val)))))
...
romaji table rather than arming colon-okurigana."
  (and (string-empty-p (nskk-state-romaji-buffer))
       (let ((start (nskk-get-conversion-start)))
         (when start
...
Returns the marker position as an integer, or nil if no marker is set."
  (when (and (markerp (nskk-state-conversion-start-marker))
...
Creates a new marker if one does not already exist."
  (let ((mk (nskk-state-conversion-start-marker)))
    (unless (markerp mk)
      (setq mk (make-marker))
      (nskk-state-set-conversion-start-marker mk))
    (set-marker mk pos (current-buffer))))
...
(defun nskk-conversion-start-active-p ()
  ...
  (and (markerp (nskk-state-conversion-start-marker))
       (marker-position (nskk-state-conversion-start-marker))))
...
Called from `nskk-undo-kakutei' after validating the saved buffer range.
...
pointed to before undo)."
  (let* ((reading      (plist-get record :reading))
         ...
         (buf-end      (+ 0 (plist-get record :buffer-end)))
...
;;;###autoload
(defun nskk-undo ()
  "Undo input according to the current conversion phase."
  (interactive)
  (cond
   ((not (string-empty-p (nskk-state-romaji-buffer)))
    ...
    (nskk-reset-romaji-buffer))
   ((nskk-converting-p)
    (if (zerop (nskk-state-current-index nskk-current-state))
        (nskk-rollback-conversion)
      (nskk-previous-candidate)))
   ((eq (nskk-state-henkan-phase nskk-current-state) 'on)
    (if (<= (point) (1+ (nskk-get-conversion-start)))
        (nskk-henkan-kakutei)
      (delete-char -1)))
   (t
    (let* ((nskk-mode nil)
           (command (key-binding (this-command-keys-vector))))
      (unless (and (commandp command) (not (eq command 'nskk-undo)))
        (setq command 'undo))
...
Reverts the committed text and searches again with the committed candidate
first, selecting the next candidate or offering dictionary registration.
If the kakutei involved a dictionary
registration, unregisters the word from the user dictionary.
The saved text range follows intervening edits.  Active conversion or
an absent record signals a user error."
  (interactive)
  (when (nskk-converting-p)
    (user-error "Cannot undo kakutei during conversion"))
  (let ((record nskk--last-kakutei-record))
    (if (null record)
        (user-error "No kakutei to undo")
      (let* ((buf-start      (plist-get record :buffer-start))
             ...
             (saved-overlay (nskk-state-conversion-overlay))
        ...
        (if (and buf-start buf-end
                 (<= (point-min) buf-start buf-end (point-max)))
            (condition-case err
...
No-op when WAS-ABBREV is nil or previous-mode is nil or abbrev."
  (when was-abbrev
    (when (fboundp 'nskk-set-numeric-mode) (nskk-set-numeric-mode nil))
...
(defun/done nskk--rollback-conversion-edit-buffer (start preedit-end saved-point)
  "Replace the ▼ marker at START with ▽, retaining the okurigana reading.
PREEDIT-END is retained for caller compatibility.  On a signal,
...
Must be called only after `nskk--rollback-conversion-edit-buffer' succeeds."
  ...
  (nskk-state-set-henkan-count 0)
  (nskk--dismiss-candidate-list)
  (nskk--run-presentation-actions 'rollback #'funcall)
  ...
  (when (nskk-converting-p)
    (let ((start (nskk-get-conversion-start)))
      ...
      (let ((preedit-end (when (overlayp (nskk-state-conversion-overlay))
                           (+ (overlay-end (nskk-state-conversion-overlay))
                              (or (overlay-get (nskk-state-conversion-overlay)
                                               'nskk-okurigana-length)
                                  0))))
            (saved-point (point)))
  ...
  (if (nskk-converting-p)
      (let ((action (nskk-prolog-query-value
        ...
        (pcase action
          ...
          ('select-prev
           (if (zerop (nskk-state-current-index nskk-current-state))
               (progn (nskk-rollback-conversion) (succeed nil))
           (when (> (nskk-state-henkan-count) 0)
             (nskk-state-set-henkan-count (1- (nskk-state-henkan-count))))
...
was made; does nothing and returns nil when START or CANDIDATE is nil."
  (when (and start candidate)
    (let ((command-boundary (and (consp buffer-undo-list)
      ...
      ;; Materializing the display belongs to the input group.  Move the
      ;; command boundary before opening a change group so rollback owns
      ;; an unchanged undo-list tail.
      (when command-boundary
      ...
      (condition-case err
          (atomic-change-group
            ...
            (when okuri-kana
              (insert okuri-kana))
            (when command-boundary (undo-boundary))
  ...
  (let* ((candidates (nskk-state-candidates nskk-current-state))
    ...
    (when (> total 0)
      (let* ((current   (nskk-state-current-index nskk-current-state))
             ...
             (end       (if (overlayp (nskk-state-conversion-overlay))
                            (overlay-end (nskk-state-conversion-overlay))
                          (point))))
...
is guaranteed valid."
  (let* ((candidates (nskk-state-candidates nskk-current-state))
         ...
         (previous-count (nskk-state-henkan-count))
         ...
         (next-start (if nskk--henkan-candidate-list-active
                         (+ current per-page)
                       (max current (1- nskk-henkan-show-candidates-nth)))))
...
is guaranteed valid."
  (let* ((candidates (nskk-state-candidates nskk-current-state))
         ...
         (per-page (min nskk-henkan-number-to-display-candidates
                        (length nskk-henkan-show-candidates-keys)))
         (first-start (1- nskk-henkan-show-candidates-nth)))
    (if (<= current first-start)
        (if (zerop current)
            (nskk-rollback-conversion)
          (nskk--dismiss-candidate-list)
          (nskk-state-set-henkan-phase nskk-current-state 'active)
          (nskk-state-set-henkan-count current)
          (nskk--select-candidate 'previous))
      (let ((prev-start (max first-start (- current per-page))))
...
QUERY is the dict lookup key stored in okurigana-query metadata."
  (let ((candidates (if (fboundp 'nskk-study-reorder)
    ...
    (let ((okuri-kana-start (- preedit-end (length nskk-okurigana-marker))))
      ...
      (overlay-put (nskk-state-conversion-overlay) 'nskk-okurigana-length
                   (- (point) okuri-kana-start)))
    (nskk-with-current-state
...
`nskk--commit-registration-word'."
  ...
  (if (< (nskk-state-registration-depth) nskk-max-registration-depth)
      (let ((state nskk-current-state)
            ...
            (prev-depth (nskk-state-registration-depth))
            (result nil)
            first-condition)
        (nskk-with-current-state
...
hook.  Assumes `nskk-current-state' is bound."
  (let ((candidates (nskk-state-candidates nskk-current-state))
    ...
    (nskk-state-set-henkan-count nskk-henkan-show-candidates-nth)
    (nskk-state-set-henkan-phase nskk-current-state 'list)
    (nskk--run-candidate-show-transaction
  ...
  (let* ((start (nskk-get-conversion-start))
    ...
    (if text
        (let* ((query (and (nskk-with-current-state
               ...
               (reading (or (and (stringp query) query)
                            (nskk-state-get-metadata
                             nskk-current-state 'henkan-reading)
                            text))
...
is the original preedit prefix for display styling."
  (when (and (boundp 'nskk-dcomp-multiple-activate)
    ...
    (let* ((after-str (nskk--dcomp-multiple-build-string
           ...
           (anchor (or (and (overlayp (nskk-state-conversion-overlay))
                            (overlay-end (nskk-state-conversion-overlay)))
                       (point))))
      (let ((ov (nskk-state-dcomp-multiple-overlay)))
        (if (overlayp ov)
...
(defun nskk--dcomp-command-p (command)
  "Return non-nil when COMMAND is a preedit completion command."
  (memq command '(nskk-handle-tab completion-at-point)))

(defun nskk--dcomp-prepare ()
...
(defun nskk--dcomp-cancel ()
  "Restore the reading when the preceding command attempted completion.
Return non-nil when completion was canceled."
  (when (and (nskk--dcomp-command-p last-command) nskk--dcomp-prefix)
    (nskk--dcomp-replace-preedit nskk--dcomp-prefix)
    (nskk--reset-dcomp-context)
    (nskk--clear-state-overlay #'nskk-state-dcomp-multiple-overlay
                               #'nskk-state-set-dcomp-multiple-overlay)
    t))
...
(defun nskk--numeric-parse-reading (reading)
  "Parse numeric READING like \"12こ\" into (NUM-STR . BASE-KEY).
Extracts leading digits (optionally after '#') and returns the number string
