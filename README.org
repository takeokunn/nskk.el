...
page (or the previous inline candidate from the first page). =RET= and
...
| =C-n=   | =nskk-handle-ctrl-n= | Next inline candidate; ignored in candidate list; otherwise commit any reading and move down |
...
These bindings leave an open candidate list unchanged.
