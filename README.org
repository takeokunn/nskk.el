...
- Clean architecture — 6-layer modular design with full lexical-binding
...
|------------------+-----------------------------------------|
| Lexical binding  | All 19 modules use =lexical-binding: t= |
| Prolog engine    | Conversion rules as facts/queries       |
| Layer separation | 6 strict layers, zero circular deps     |
...
stateDiagram-v2
    ...
    Conversion --> Conversion: SPC
    Conversion --> Conversion: x
    Conversion --> Conversion: a-l
    Conversion --> ASCII: RET
    Conversion --> ASCII: C-g
    Conversion --> ASCII: arrow
    Conversion --> ASCII: C-a
    Conversion --> ASCII: C-e
...
** Hooks
#+begin_src elisp
;; Run when NSKK is enabled in a buffer
(add-hook 'nskk-mode-hook
          (lambda () (message "NSKK activated")))

;; Run when NSKK is disabled
(add-hook 'nskk-mode-off-hook
          (lambda () (message "NSKK deactivated")))

;; Run on every input-mode change (ascii → hiragana, etc.)
(add-hook 'nskk-input-mode-hook #'my-nskk-mode-change-handler)
#+end_src
** Customization Group
#+begin_example
