...
(defconst nskk--pbt-test-pred
  'nskk-pbt-test-dict-entry
...
(nskk-describe "dictionary load fixture"
               (nskk-it
                "should load test dictionary and expose all known keys via Prolog"
                (nskk-prolog-test-with-isolated-db (nskk-prolog-clear-database)
                                                   (let* ((dict-path
                                                           (nskk--pbt-fixture-dict-path))
                                                          (pred
                                                           (nskk-dict-load-file
                                                            dict-path
                                                            nil
                                                     ...
                                                     (let ((failures nil))
                                                       ...
                                                       (when failures
                                                         (ert-fail
                                                          (format
                                                           "Missing keys in loaded dictionary: %S"
                                                           failures))))))))
...
(nskk-property-test dict-save-load-roundtrip
                    ((key known-dict-key) (candidates candidate-list))
                    (nskk-prolog-test-with-isolated-db
                     (let ((temp-file
                            (make-temp-file "nskk-test-dict-" nil ".skk")))
                       (unwind-protect
                           (progn
                             (nskk-prolog-retract-all 'user-dict-entry 2)
                             (nskk-prolog-set-index 'user-dict-entry 2 :trie)
                             (nskk-prolog-assert
                              (list (list 'user-dict-entry key candidates)))
                             (let ((nskk--user-dict-index 'user)
                               ...
                               (let* ((reload-pred 'nskk-pbt-reload-dict)
                                      (_
                                       (nskk-prolog-retract-all reload-pred 2))
                                      (loaded
                                       (nskk-dict-load-file temp-file
                                                            nil
                                                            reload-pred)))
                                 ...
                                 (let ((reloaded-candidates
                                        (nskk-prolog-query-value
                                         ...
                                         '\?c)))
                                   (should
                                    (equal reloaded-candidates candidates))))))
...
(nskk-deftest-table dict-known-entries
                    ...
                    (("かんじ" "漢字") ("うみ" "海"))
                    :body
                    (nskk-with-mock-dict (list (cons input (list expected)))
                                         (should
                                          (member expected
                                                  (nskk-dict-lookup input)))))
