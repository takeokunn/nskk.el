...
(declare-function nskk-cps-test--if-multi-else nil)
(declare-function nskk-cps-test--if-multi-else/k nil)
...
(declare-function nskk-cps-test--or-done-guard nil)
(declare-function nskk-cps-test--or-done-guard/k nil)
...
(nskk-describe "<-or macro"
  ...
  (nskk-context "error conditions"
    ...
    (nskk-it "signals an error at macroexpand time when fn was generated by defun/done"
      (defun/done nskk-cps-test--or-done-guard ()
        "Test-only side-effect function for the <-or guard test."
        nil)
      (should-error
       (macroexpand '(defun/k nskk-cps-test--or-guard-trigger ()
                       "Doc."
                       (<-or r nskk-cps-test--or-done-guard
                             :found (succeed r)
                             :fail  (fail))))
       :type 'error))))
...
(nskk-describe "CPS walker"
  ...
  (nskk-context "if form"
    ...
    (nskk-it "transforms the then-branch of a one-branch if leaving no else clause"
      (let* ((expansion (macroexpand-1
        ...
        (should (= (length if-form) 3))))

    (nskk-it "runs every form of a multi-form else and returns the last one's value"
      (let ((else-side-effect nil))
        (defun/k nskk-cps-test--if-multi-else (x)
          "Succeed with X, else record a side effect then succeed with :last."
          (if x
              (succeed x)
            (setq else-side-effect :ran)
            (succeed :last)))
        (should (eq (nskk-cps-test--if-multi-else t) t))
        (should (null else-side-effect))
        (should (eq (nskk-cps-test--if-multi-else nil) :last))
        (should (eq else-side-effect :ran))
        (setq else-side-effect nil)
        (let ((found nil)
              (not-found-called nil))
          (nskk-cps-test--if-multi-else/k nil
                                          (lambda (v) (setq found v))
                                          (lambda () (setq not-found-called t)))
          (should (eq found :last))
          (should (eq else-side-effect :ran))
  ...
  (nskk-context "cond test-only clause edge cases"
    ...
    (nskk-it "a malformed non-cons cond clause is emitted as written, not dropped"
      (let* ((expansion (macroexpand-1
                         '(defun/k nskk--cps-test-cond-malformed (x)
                            "Doc."
                            (cond (x (succeed x)) 42))))
             (k-def     (nth 1 expansion))
             (cond-form (car (nthcdr 4 k-def))))
        (should (eq (car cond-form) 'cond))
        (should (= (length cond-form) 3))
        (should (equal (nth 2 cond-form) 42))))))
...
;;; -------------------------------------------------------------------
;;; nskk-<-or standalone pipeline-bind macro
;;; -------------------------------------------------------------------
...
(nskk-describe "defun/k <-seq special form"
  ...
  (nskk-context "malformed binding vector"
    (nskk-it "signals when the call slot is not a call form"
      (should-error
       (macroexpand '(defun/k nskk--cps-test-seq-bad-call ()
                       "Doc."
                       (<-seq [v not-a-call] (succeed v))))
       :type 'error))

    (nskk-it "signals when the variable slot is not a symbol"
      (should-error
       (macroexpand '(defun/k nskk--cps-test-seq-bad-var ()
                       "Doc."
                       (<-seq [42 (my-fn)] (succeed 42))))
       :type 'error))))
