...
```elisp
;; NSKKの基本設定
(setopt nskk-jisyo-file "~/.skk/jisyo")  ; 個人辞書のパス
```
...
```elisp
;; NSKKの推奨設定
(setopt nskk-jisyo-file "~/.skk/jisyo")        ; 個人辞書
(setopt nskk-show-mode-show t)                 ; モード表示を有効化
(setopt nskk-modeline-indicator-enabled t)     ; モードライン表示
```
...
```elisp
;; システム辞書の設定（任意）
(setopt nskk-large-jisyo "~/.skk/SKK-JISYO.L")
```
...
2. **モードライン表示の有効化**
   ```elisp
   (setopt nskk-modeline-indicator-enabled t)
   ```
...
;; 高速化設定
(setopt nskk-use-cache t)
(setopt nskk-preload-dictionary t)
```
...
3. **エンコーディングの確認**
   ...
   (setopt nskk-dictionary-encoding 'utf-8)
   ```
...
;; init.elで明示的に設定
(keymap-global-set "C-x C-j" #'nskk-mode)
(keymap-set nskk-mode-map "C-j" #'nskk-to-hiragana-mode)
```
...
1. **メモリ使用量の確認**
   ```elisp
   ;; memory-usageはGNU ELPAの外部パッケージです。
   ;; M-x package-install RET memory-usage RET でインストール可能。
   M-x memory-usage
   ```
...
3. **[アーキテクチャガイド](../explanation/architecture-v0.1.md)**
   - NSKKの内部構造
   - モジュール間の関係
   - 拡張方法
