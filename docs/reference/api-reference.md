...
- **高速パフォーマンス**: コア関数の応答 < 1ms を目標
- **並列処理**: スレッドプールによる並列実行
- **非同期API**: コールバック対応
- **テストカバレッジ**: Property-based testing統合
...
(nskk--load-dictionary)
```
Data Access Layer を初期化し、利用可能な辞書インデックスを返します。

**戻り値**: ロード済み辞書インデックス（存在しない場合は `nil`）
**副作用**: `nskk-layer-data` の辞書設定が初期化されます

**例外**: 辞書パスが無効な場合でも例外は発生せず、`nil` を返します
...
**副作用**: Data Layer を通じて学習情報が適用されます
...
以下の機能はEmacs バージョンに依存します（導入時期の参考情報）：
- native-compilation: Emacs 28.1で導入
- setopt: Emacs 29.1で導入
- keymap-set / keymap-global-set: Emacs 29.1で導入
