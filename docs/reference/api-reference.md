...
NSKKが提供する全ての公開関数、変数、マクロの完全なリファレンスです。世界最高峰のSKK実装として、全APIがEmacs Lispベストプラクティスに従って設計されています。

### API設計原則
- **ゼロ依存**: Emacs標準機能のみ使用
- **マクロ最適化**: コンパイル時最適化で実行時オーバーヘッドゼロ
- **型安全性**: cl-lib活用による強型システム
- **パフォーマンス**: 全関数が1ms以下の応答時間を保証
- **完全性**: 全公開APIが100%テストカバレッジ
