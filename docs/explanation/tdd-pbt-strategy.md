...
graph LR
    ...
    subgraph "コードカバレッジ目標"
        STMT[文カバレッジ<br/>目標 90%]
        BRANCH[分岐カバレッジ<br/>目標 85%]
        FUNC[関数カバレッジ<br/>目標 95%]
    end

    subgraph "プロパティカバレッジ目標"
        IDEM[冪等性<br/>目標 100%]
        REV[可逆性<br/>目標 85%]
        CONS[整合性<br/>目標 90%]
    end
...
(defun nskk--save-test-data (data)
  "テストデータの永続化
with-temp-file はbody実行後にバッファ内容を指定ファイルに書き出す。
prin1でdataをカレントバッファに出力し、それがファイルに保存される。"
...
NSKKのTDD/PBT戦略により、以下の品質目標を目指します：
...
この戦略に従うことで、外部依存なしでありながら、高品質なSKK実装を実現することを目指します。