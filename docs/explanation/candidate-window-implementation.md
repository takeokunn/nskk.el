...
`nskk-candidate-window.el` は、NSKK の候補ウィンドウ機能の設計仕様です。overlayベースのポップアップ表示により、外部依存なしで候補の表示・選択・ページング機能を提供します。
...
1. **データ正規化**: 候補データの正規化処理
2. **番号フォーマット**: 候補番号の表示
3. **候補フォーマット**: 単一候補の整形
4. **ページフォーマット**: ページ全体の整形
5. **表示位置計算**: 位置算出ロジック
6. **表示・非表示**: ウィンドウの表示/非表示制御
7. **候補更新**: 候補リストの動的更新
8. **スクロール**: ページング動作
9. **候補選択**: 選択操作
10. **情報取得**: 状態取得API
11. **ユーティリティ**: ヘルパー関数
12. **overlay操作**: overlay生成・更新・削除
13. **統合テスト**: 複数機能の連携
...
1. **アニメーション**: Emacsにはoverlayのスムーズなアニメーション機構がないため、タイマーによるステップ的な表示切替で擬似的な効果を検討。実現可能性は実験的。
2. **マルチカラム**: 横方向の候補配置
3. **ソート**: 頻度ベース・優先度ベース
