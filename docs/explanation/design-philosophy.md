...
入力メソッドにおいて、反応速度は直接的にユーザー体験を左右します。NSKKは高速な応答性能を設計目標としています：
...
NSKKでは用途に応じて `cl-defstruct` と plist を使い分けます。内部的なパフォーマンスクリティカルな構造体（トライ木ノード、オブジェクトプール等）には `cl-defstruct` を採用し、型安全性とアクセサの自動生成を活用します。一方、設定データやシリアライズが必要なデータにはplistを使用します。

```elisp
;; cl-defstruct: 内部データ構造向き（高速アクセス、型安全）
(cl-defstruct nskk-trie-node
  children is-terminal candidates)

;; plist: 設定・永続化データ向き（柔軟性、可読性）
(list :text "候補" :score 100 :source 'dictionary)
```

使い分けの基準：
- **cl-defstruct**: パフォーマンスクリティカル、構造が固定、内部使用
- **plist**: シリアライズが必要、動的フィールド追加、デバッグ時の可読性重視
